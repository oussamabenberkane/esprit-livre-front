...
const BookCard = ({
    ...
    return (
        <div className="bg-white rounded-2xl shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden group w-70 h-110 relative">
            ...
            <div className="relative  w-full h-62 overflow-hidden flex items-end justify-center">
                <img
                    src={coverImage}
                    alt={title}
                    className="h-[95%] bottom-0 object-cover group-hover:scale-105 transition-transform duration-300"
                />

                {/* Badge */}
                {badge && (
                    <div className={`absolute top-0 left-0 px-3 py-2 rounded-br-2xl text-xs font-semibold ${getBadgeStyles(badge.type)}`}>
                        {badge.text}
                    </div>
                )}
