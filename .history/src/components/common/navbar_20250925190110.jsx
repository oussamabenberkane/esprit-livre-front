...
const SearchBar = ({ placeholder = "Recherchez un livre, un genre ..etc", onSearch }) => (
    <div className="relative bg-white rounded-lg h-11 w-140 flex items-center">
        ...
        <input
            ...
            onChange={onSearch}
            className="w-full h-full pl-10 pr-4 text-md text-slate-500 bg-transparent border-none outline-none placeholder:text-slate-500"
        />
    </div>
);
...
const LanguageSelector = ({ onLanguageChange }) => (
    <div
        className="relative bg-white rounded h-10 w-25 flex items-center cursor-pointer hover:bg-gray-50 transition-colors"
        onClick={onLanguageChange}
    >
        <span className="text-md text-slate-500 text-black ml-2">Langue</span>
