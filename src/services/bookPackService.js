...
const API_BASE_URL = 'https://sharp-crabs-allow.loca.lt/api';
...
export const getAllBookPacks = async (params = {}) => {
  try {
    ...
    const url = `${API_BASE_URL}/book-packs${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
...
export const getBookPackById = async (id) => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/book-packs/${id}`, {
      method: 'GET',
      headers,
    });
...
export const getRecommendedPacksForBook = async (bookId, params = {}) => {
  try {
    ...
    const url = `${API_BASE_URL}/books/${bookId}/recommended-packs${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
